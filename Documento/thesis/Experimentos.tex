...
\subsection{Proyección en tiempo real de instancias óptimas}
El protocolo de captura comienza marcando en el piso las instancias óptimas obtenidas mediante el algoritmo descrito en la sección \ref{secGen}, para ello se desarrolló un programa que recibe como entrada las instancias de marcas y las proyecta y pinta en tiempo real sobre lo que esté capturando la cámara, esto quiere decir que en la escena (a partir de la cual se estimó el plano del piso) se dibujan en la imagen las marcas óptimas con lo cual se tiene una referencia al momento de marcarlas físicamente en el piso del laboratorio de experimentación.\\

Como se ha mencionado los puntos se eligieron tomando en cuenta una región limitada en base a análisis previos y con la restricción entre otras 
%Agregar dibujo que represente el problema del campo visual
